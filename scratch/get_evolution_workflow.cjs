...
async function run() {
  ...
  if (res.statusCode === 200) {
    fs.writeFileSync('scratch/evolution_workflow_backup.json', JSON.stringify(res.data, null, 2));
    console.log("Success! Saved current evolution workflow to scratch/evolution_workflow_backup.json");
    console.log("Active state:", res.data.active);
  } else {
    console.log("Error getting evolution workflow:", res.statusCode, res.data);
  }
}
