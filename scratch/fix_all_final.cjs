...
function apiCall(method, path, body) {
  return new Promise((resolve, reject) => {
    const opt = {
      method, hostname: "n8n-n8n.rh3fr2.easypanel.host",
      path: "/api/v1" + path,
...
function sendPost(url, body) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({ method:'POST', hostname:u.hostname, path:u.pathname, headers:{'Content-Type':'application/json','Content-Length':Buffer.byteLength(s)} }, (res) => {
...
// ===== IF node v2.2 format (PROVEN WORKING) =====
function makeIfNodeV2(id, name, position, leftValue, rightValue, operatorType, operation) {
  return {
    parameters: {
      conditions: {
        ...
        conditions: [{
          ...
          operator: { type: operatorType, operation: operation }
        }],
        combinator: "and"
      },
      options: {}
...
return [{ json: {
  channel_id: body.channel_id, contact_id: body.contact_id,
  provider: channel?.provider || 'unknown', content: body.content, phone: body.phone,
  phone_id: channel?.phone_id, access_token: channel?.access_token,
  evo_url: channel?.url, evo_instance: channel?.instance, evo_api_key: channel?.api_key
}}];`;

async function fixOutbound() {
  const wfId = "NFkf4R8DDJ2o7Sqx";
  console.log("=== Fixing OUTBOUND ===");
  await apiCall("POST", `/workflows/${wfId}/deactivate`);
  ...
  const data = {
    ...
    nodes: [
      ...
      makeIfNodeV2("7ee5a4e9-db7b-4331-82dd-bf8b938158db", "Which Provider?1", [1680,928], "={{ $json.provider }}", "meta", "string", "equals"),
      ...
      { parameters:{ method:"POST", url:`${supabaseUrl}/rest/v1/messages`, sendHeaders:true, headerParameters:{parameters:[{name:"apikey",value:serviceKey},{name:"Authorization",value:`Bearer ${serviceKey}`},{name:"Content-Type",value:"application/json"},{name:"Prefer",value:"return=representation"}]}, sendBody:true, bodyParameters:{parameters:[{name:"channel_id",value:"={{ $json.channel_id || $input.first().json.channel_id }}"},{name:"contact_id",value:"={{ $json.contact_id || $input.first().json.contact_id }}"},{name:"direction",value:"out"},{name:"content",value:"={{ $json.content || $input.first().json.content }}"},{name:"content_type",value:"text"},{name:"status",value:"sent"},{name:"timestamp",value:"={{ new Date().toISOString() }}"}]}, options:{} }, id:"8f054d66-7182-4dfb-bcb9-5c2b9617b361", name:"Log Outgoing Message1", type:"n8n-nodes-base.httpRequest", typeVersion:3, position:[2160,928] },
    ...
    connections: {
      "Send Message Trigger":{main:[[{node:"Fetch Channel",type:"main",index:0}]]},
      "Fetch Channel":{main:[[{node:"Resolve Channel",type:"main",index:0}]]},
      "Resolve Channel":{main:[[{node:"Which Provider?1",type:"main",index:0}]]},
      "Which Provider?1":{main:[[{node:"Send via Meta API1",type:"main",index:0}],[{node:"Send via Evolution API1",type:"main",index:0}]]},
      "Send via Meta API1":{main:[[{node:"Log Outgoing Message1",type:"main",index:0}]]},
      "Send via Evolution API1":{main:[[{node:"Log Outgoing Message1",type:"main",index:0}]]},
      "Log Outgoing Message1":{main:[[{node:"Response OK1",type:"main",index:0}]]}
    },
    settings: {}
  };

  const r = await apiCall("PUT", `/workflows/${wfId}`, data);
  console.log("Update:", r.statusCode);
  if (r.statusCode !== 200) { console.error(JSON.stringify(r.data)); return false; }
  const a = await apiCall("POST", `/workflows/${wfId}/activate`);
  console.log("Activate:", a.statusCode);
  if (a.statusCode !== 200) { console.error(JSON.stringify(a.data)); return false; }
  console.log("OUTBOUND OK!");
  return true;
}

// Also fix the inbound IF nodes to v2.2
...
return [{ json: { whatsapp_msg_id:msgId, phone, contact_name:contactName, direction:'in', content, content_type:type, media_url:null, media_id:mediaId, timestamp:new Date(parseInt(message.timestamp)*1000).toISOString() } }];`;

const metaChannelId = "4886443e-4996-4d2a-83e1-d96f503e1a28";

async function fixMeta() {
  const wfId = "88zOQbdJAT7DOaET";
  console.log("\n=== Fixing META ===");
  await apiCall("POST", `/workflows/${wfId}/deactivate`);
  ...
  const data = {
    ...
    nodes: [
      ...
      makeIfNodeV2("d9c2d088-ac0d-42ce-b58e-ab11dc919585", "Has Message?", [700,300], "={{ $json.skip }}", "", "boolean", "true"),
...
if (event === 'messages.upsert') {
  const msg = body.data; const key = msg.key; const isFromMe = key.fromMe;
  ...
  return [{ json: { event_type:'message.received', whatsapp_msg_id:key.id, phone, contact_name:pushName, direction:isFromMe?'out':'in', content, content_type:contentType, media_url:mediaUrl, timestamp:new Date(msg.messageTimestamp*1000).toISOString(), instance:body.instance }}];
} else return [{ json: { skip:true, reason:'Not a message: '+event }}];`;

const evoChannelId = "50df1e49-8f4c-4f90-b3c5-e9b95e37d8ed";

async function fixEvolution() {
  ...
  const data = {
    ...
    nodes: [
      ...
      makeIfNodeV2("91b9cbe2-6116-42e3-a45e-82eee5ae48f5", "Is Message?", [-32,1168], "={{ $json.skip }}", "", "boolean", "true"),
    ...
    connections: {
      "Evolution Webhook Trigger":{main:[[{node:"Parse Evolution Payload",type:"main",index:0}]]},
      "Parse Evolution Payload":{main:[[{node:"Is Message?",type:"main",index:0}]]},
      "Is Message?":{main:[[],[{node:"Upsert Contact",type:"main",index:0}]]},
      "Upsert Contact":{main:[[{node:"Insert Message to Supabase",type:"main",index:0}]]},
      "Insert Message to Supabase":{main:[[{node:"Log Webhook Event",type:"main",index:0}]]}
    },
    settings: {}
...
async function testAll() {
  ...
  // Test Meta inbound
  console.log("\n1. Meta inbound POST...");
  const m = await sendPost("https://n8n-n8n.rh3fr2.easypanel.host/webhook/webhook/meta", {
    object:"whatsapp_business_account", entry:[{id:"t",changes:[{value:{messaging_product:"whatsapp",metadata:{display_phone_number:"5511999999999",phone_number_id:"t"},contacts:[{profile:{name:"TesteFinal"},wa_id:"5511999999999"}],messages:[{from:"5511999999999",id:"wamid.FINAL_"+Date.now(),timestamp:String(Math.floor(Date.now()/1000)),text:{body:"Meta final test "+new Date().toLocaleTimeString()},type:"text"}]},field:"messages"}]}]
  });
  console.log(`   Status: ${m.statusCode} Body: ${m.body}`);

  // Test Evolution inbound
  console.log("\n2. Evolution inbound POST...");
  const e = await sendPost("https://n8n-n8n.rh3fr2.easypanel.host/webhook/webhook/evolution", {
    event:"messages.upsert", instance:"EvoInst",
    data:{ key:{remoteJid:"5511988888888@s.whatsapp.net",fromMe:false,id:"EVO_FINAL_"+Date.now()}, pushName:"TesteFinalEvo", message:{conversation:"Evo final test "+new Date().toLocaleTimeString()}, messageTimestamp:Math.floor(Date.now()/1000) }
  });
  console.log(`   Status: ${e.statusCode} Body: ${e.body}`);

  // Test Outbound
  console.log("\n3. Outbound /webhook/send...");
  const o = await sendPost("https://n8n-n8n.rh3fr2.easypanel.host/webhook/send", {
    channel_id: metaChannelId, phone: "5511999999999", content: "Outbound final test " + new Date().toLocaleTimeString()
  });
  console.log(`   Status: ${o.statusCode} Body: ${o.body}`);

  // Wait and check results
  console.log("\nWaiting 5s for executions...");
  await new Promise(r => setTimeout(r, 5000));

  for (const wfId of ["88zOQbdJAT7DOaET", "m5wmXXTYAqLiRM9c", "NFkf4R8DDJ2o7Sqx"]) {
    const result = await apiCall("GET", `/executions?workflowId=${wfId}&limit=1`);
    const ex = result.data?.data?.[0];
    if (ex) {
      ...
      if (ex.status === 'error') {
        ...
        for (const [n,ne] of Object.entries(rd.runData||{})) { for (const x of ne) { if(x.error) console.log(`  Node "${n}": ${x.error.message}`); }}
      }
    }
  }
  ...
  const supaResp = await new Promise((resolve, reject) => {
    ...
    https.get(`${supabaseUrl}/rest/v1/messages?created_at=gte.${ago}&order=created_at.desc&limit=10&select=id,content,direction,channel_id`, { headers:{'apikey':serviceKey,'Authorization':`Bearer ${serviceKey}`} }, (res) => {
...
async function run() {
  const ok1 = await fixMeta();
  const ok2 = await fixEvolution();
  const ok3 = await fixOutbound();
  if (ok1 && ok2 && ok3) await testAll();
  else console.error("Some patches failed!");
}
