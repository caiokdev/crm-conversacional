...
async function run() {
  const result = await apiCall("/executions?workflowId=m5wmXXTYAqLiRM9c&limit=5");
  const execs = result.data || [];
  console.log("Recent Evolution Executions:");
  for (const ex of execs) {
    console.log(`\n=== Exec ${ex.id} (${ex.startedAt}) ===`);
    const detail = await apiCall(`/executions/${ex.id}?includeData=true`);
    const rd = detail.data?.resultData || {};
    if (rd.runData && rd.runData['Parse Evolution Payload']) {
      const parseOut = rd.runData['Parse Evolution Payload'][0]?.data?.main?.[0]?.[0]?.json;
      console.log("Parsed Output:", JSON.stringify(parseOut, null, 2));
    }
  }
}
